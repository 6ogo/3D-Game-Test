...
export class ProjectileManager {
  ...
  private projectiles: Map<THREE.Object3D, {
   ...
   * Check for collision
   */
  private checkCollision(projectile: THREE.Object3D, owner: 'player' | 'enemy'): THREE.Object3D | null {
    ...
    for (const target of targets) {
      ...
      if (distance < 1) {  // 1 unit collision radius
        return target;
      }
    }
   ...
   * Handle a hit
   */
  private handleHit(projectile: THREE.Object3D, hit: THREE.Object3D, data: any): void {
   ...
   * Spawn a hit effect
   */
  private spawnHitEffect(position: THREE.Vector3, owner: 'player' | 'enemy'): void {
    // This would use the ParticleSystem in a real implementation
    // For now, just log it
    console.log(`Spawn hit effect at ${position.x}, ${position.y}, ${position.z}`);
  }
...
export class ParticleSystem {
  ...
  private activeParticles: Map<THREE.Object3D, {
    lifespan: number;
    spawnTime: number;
    velocity: THREE.Vector3;
    type: string;
  ...
  emitParticles(
    ...
    switch (type) {
      case 'hit': pool = this.hitParticlePool; break;
      case 'dash': pool = this.dashParticlePool; break;
      case 'death': pool = this.deathParticlePool; break;
      case 'fire': pool = this.fireParticlePool; break;
      default: pool = this.hitParticlePool;
    }
    ...
    for (let i = 0; i < count; i++) {
      ...
      particle.position.copy(position).add(new THREE.Vector3(offsetX, offsetY, offsetZ));
      ...
      if (color && particle instanceof THREE.Mesh) {
        ...
        if (particle.children.length > 0 && particle.children[0] instanceof THREE.PointLight) {
  ...
  private getPoolForType(type: string): ObjectPool<THREE.Object3D> {
    switch (type) {
      case 'hit': return this.hitParticlePool;
      case 'dash': return this.dashParticlePool;
      case 'death': return this.deathParticlePool;
      case 'fire': return this.fireParticlePool;
      default: return this.hitParticlePool;
    }
  }
   ...
   */
  createHitEffect(position: THREE.Vector3, color: THREE.Color = new THREE.Color(0xff0000)): void {
  ...
  createDashEffect(position: THREE.Vector3, direction: THREE.Vector3): void {
    ...
    for (let i = 0; i < stepCount; i++) {
      const t = i / stepCount;
      const pos = position.clone().add(direction.clone().multiplyScalar(t * dashLength));
...
export class EnemyPool {
  ...
  // Active enemies
  private activeEnemies: Map<THREE.Object3D, {
    type: string;
    health: number;
    maxHealth: number;
    id: string;
  ...
  private createEnemy(type: string): THREE.Object3D {
    ...
    switch (type) {
      ...
      case 'elite':
        ...
        const leftEye = new THREE.Mesh(new THREE.SphereGeometry(0.1, 8, 8), eyesMaterial);
        const rightEye = new THREE.Mesh(new THREE.SphereGeometry(0.1, 8, 8), eyesMaterial);
      ...
      case 'boss':
        ...
        const core = new THREE.Mesh(new THREE.SphereGeometry(0.3, 16, 16), glowMaterial);
        core.position.y = 1.2;
  ...
  applyDamage(enemy: THREE.Object3D, damage: number): boolean {
    ...
    if (data.health <= 0) {
      // Release back to pool
      switch (data.type) {
        case 'minion': this.minionPool.release(enemy); break;
        case 'elite': this.elitePool.release(enemy); break;
        case 'boss': this.bossPool.release(enemy); break;
      }
      return true;
    }
