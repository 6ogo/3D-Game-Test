...
export class LevelManager {
  ...
  private async preloadAssets(): Promise<void> {
    ...
    const floorTexture = await textureLoader.loadAsync(
      "/textures/floor_diffuse.png"
    );
    const wallTexture = await textureLoader.loadAsync(
      "/textures/wall_diffuse.png"
    );
    const normalMapTexture = await textureLoader.loadAsync(
  ...
  async activateRoom(roomId: string): Promise<void> {
    ...
    const room = this.currentLevel.rooms.find(
    ...
    const adjacentRoomIds = room.connections.map(
      (conn: { targetRoomId: any }) => conn.targetRoomId
    );

    // Determine which rooms to load, keep, and unload
    const roomsToLoad = [roomId, ...adjacentRoomIds].filter(
      (id) => !this.loadedRooms.has(id)
    );
    const roomsToKeep = [roomId, ...adjacentRoomIds];
    const roomsToUnload = Array.from(this.loadedRooms.keys()).filter(
      (id) => !roomsToKeep.includes(id)
    );
  ...
  private async loadRoom(roomId: string): Promise<Room> {
    ...
    const loadPromise = new Promise<Room>(async (resolve) => {
      ...
      // Find room data
      const roomData = this.currentLevel.rooms.find(
        (r: { id: string }) => r.id === roomId
      );
      if (!roomData) {
  ...
  private disposeRoomResources(room: Room): void {
    ...
    room.entities!.forEach((_entity: any) => {
  ...
  private generateLevelData(levelId: string): LevelData {
    ...
    for (let i = 1; i < roomCount - 1; i++) {
      const roomType: RoomType =
        i % 3 === 0 ? "elite" : i % 5 === 0 ? "treasure" : "normal";
  ...
  private generatePropPositions(roomData: any): PropData[] {
    ...
    for (let i = 0; i < propCount; i++) {
      ...
      const distance =
        10 +
        this.noise2D(Math.cos(angle) * roomSeed, Math.sin(angle) * roomSeed) *
          5;
