...
export class VisualEffectsManager {
  ...
  static getInstance(renderer?: THREE.WebGLRenderer, scene?: THREE.Scene, camera?: THREE.Camera): VisualEffectsManager {
    if (!VisualEffectsManager.instance && renderer && scene && camera) {
      VisualEffectsManager.instance = new VisualEffectsManager(renderer, scene, camera);
    }
   ...
   */
  setupEnvironmentLighting(type: 'normal' | 'elite' | 'boss' = 'normal'): void {
    ...
    this.environmentLighting.ambientLight = new THREE.AmbientLight(ambientColor, ambientIntensity);
    this.scene.add(this.environmentLighting.ambientLight);
    
    // Create directional light with shadows
    this.environmentLighting.directionalLight = new THREE.DirectionalLight(directionalColor, directionalIntensity);
    ...
    this.environmentLighting.directionalLight.shadow.camera.left = -shadowCamSize;
    this.environmentLighting.directionalLight.shadow.camera.right = shadowCamSize;
    this.environmentLighting.directionalLight.shadow.camera.top = shadowCamSize;
    this.environmentLighting.directionalLight.shadow.camera.bottom = -shadowCamSize;
   ...
   */
  private addAtmosphericLight(color: number, intensity: number, x: number, y: number, z: number, decay: number, distance: number): void {
   ...
   */
  flashScreen(color: THREE.Color | number | string = 0xffffff, intensity: number = 1.0, duration: number = 0.3): void {
  ...
  private updateScreenFlash(deltaTime: number): void {
    ...
    if (flashPass) {
      ...
      flashPass.uniforms.offset.value = originalOffset * (1 - this.screenFlash.intensity * 0.5);
    }
  }
   ...
   */
  createAttackTrail(startPoint: THREE.Vector3, endPoint: THREE.Vector3, color: THREE.Color | number | string, width: number = 1.0): void {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      this.particleSystem.emitParticles('dash', pos, 1, 300, 0.2, 0.05, new THREE.Color(color));
    }
  }
   ...
   */
  createImpactEffect(position: THREE.Vector3, color: THREE.Color | number | string, size: number = 1.0): void {
    // Emit particles in an explosion pattern
    this.particleSystem.emitParticles('hit', position, 15, 500, size, 0.2, new THREE.Color(color));
   ...
   */
  createDeathEffect(position: THREE.Vector3, type: 'normal' | 'elite' | 'boss'): void {
    ...
    this.particleSystem.emitParticles('death', position, particleCount, 1000, size, 0.3, color);
    
    // Add some fire particles
    this.particleSystem.emitParticles('fire', position, Math.floor(particleCount * 0.7), 800, size * 0.8, 0.2);
  ...
  createHealEffect(position: THREE.Vector3, amount: number): void {
    ...
    this.particleSystem.emitParticles('heal', position, 15, 1000, size, 0.05, color);
   ...
   */
  createBuffEffect(target: THREE.Object3D, color: THREE.Color | number | string, duration: number): void {
    ...
    const emitBuffParticles = () => {
      ...
      for (let i = 0; i < particleCount; i++) {
        ...
        const particlePos = position.clone().add(new THREE.Vector3(offsetX, 0, offsetZ));
        this.particleSystem.emitParticles('buff', particlePos, 1, 500, 0.3, 0.05, bufColor);
      }
  ...
  private updateEnvironmentLights(deltaTime: number): void {
    ...
    this.environmentLighting.pointLights.forEach((light, index) => {
      ...
      // Apply flicker
      light.intensity = (light.userData.originalIntensity || light.intensity) * (1 - flickerAmount + flickerAmount * flicker);
    });
  }
  ...
  updateEffectSettings(settings: any): void {
    ...
    if (settings.bloom) {
      ...
      if (bloomPass) {
        ...
        if (settings.bloom.threshold !== undefined) bloomPass.threshold = settings.bloom.threshold;
      }
    ...
    if (settings.chromaticAberration) {
      ...
      if (caPass) {
        if (settings.chromaticAberration.strength !== undefined) {
          caPass.uniforms.aberrationStrength.value = settings.chromaticAberration.strength;
        }
      }
      
      this.chromaticAberrationParams = { ...this.chromaticAberrationParams, ...settings.chromaticAberration };
    }
    ...
    if (settings.vignette) {
      ...
      if (vignettePass) {
        if (settings.vignette.offset !== undefined) vignettePass.uniforms.offset.value = settings.vignette.offset;
        if (settings.vignette.darkness !== undefined) vignettePass.uniforms.darkness.value = settings.vignette.darkness;
      }
